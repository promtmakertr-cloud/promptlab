...
// 🔥 VİZYONER PROMPT HAVUZU (Birebir Korundu) 🔥
const allPrompts = [
...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    } catch (err) { alert("Hata"); setInput(currentInput); } 
  ...
  // 🔥 NÜKLEER ÇÖZÜM: NEXT.JS ROUTER'I TAMAMEN ÖLDÜRME 🔥
  const hardRedirect = (targetUrl, platformName) => {
    try {
      navigator.clipboard.writeText(result);
      setCopyStatus(platformName + ' Açılıyor...');
    } catch (e) {}

    // Next.js Link mekanizmasını atlatmak için window.location.assign kullanıyoruz.
    // URL'deki muhtemel gizli boşlukları ve markdown kalıntılarını temizler.
    const cleanUrl = targetUrl.replace(/[\[\]\s]/g, "");
    
    // Eğer tarayıcıda hala 404 alıyorsak, target="_blank" ile yeni sekmede zorluyoruz.
  ...
  useEffect(() => {
    ...
    styleSheet.innerText = `
      @keyframes elegantGlow {
        0%   { box-shadow: 0 0 8px rgba(255, 191, 0, 0.1), border-color: rgba(255, 191, 0, 0.15); }
        50%  { box-shadow: 0 0 20px rgba(255, 126, 0, 0.25), border-color: rgba(255, 126, 0, 0.35); }
        100% { box-shadow: 0 0 8px rgba(255, 191, 0, 0.1), border-color: rgba(255, 191, 0, 0.15); }
      }
      ...
      .loading-box { width: 100%; max-width: 600px; background: rgba(10, 10, 10, 0.8); border: 1px solid rgba(255, 191, 0, 0.3); border-radius: 16px; padding: 40px 20px; text-align: center; }
      .loading-text { font-size: 1.1rem; color: #ffbf00; margin-top: 15px; }
      ...
      .cinematic-text:hover .prompt-category { color: #ffbf00; }
      .prompt-category { font-style: italic; color: #fff; opacity: 0.95; margin-bottom: 4px; }
      .ai-brand-btn { display: inline-flex; align-items: center; gap: 8px; background: rgba(20, 20, 20, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); color: #d1d1d1; padding: 10px 18px; border-radius: 12px; cursor: pointer; transition: all 0.3s; font-family: inherit; }
      ...
      @media (max-width: 768px) { .hero-section { margin-top: 42vh !important; } .cinematic-text { font-size: 0.85rem !important; } }
    `;
  ...
  return (
    <main style={container}>
      <div style={topBar}>
        ...
        {submittedPrompt && <button onClick={handleReset} style={backButton}>← Geri Dön</button>}
      </div>
          ...
          <>
            <div style={floatingContainer}>
              {slots.map((slot) => {
                ...
                return (
                  <div key={slot.id} className="cinematic-text" onClick={() => setInput(promptText)} onAnimationIteration={() => handleAnimationIteration(slot.id)}
            ...
            <div style={heroSection} className="hero-section">
              ...
              <p style={heroSub}>Arka planda AI ile işlenen profesyonel promptlar.</p>
            </div>
          </>
        ) : (
          <div style={resultContainer}>
             <div style={userPromptWrapper}>
               <div style={userPromptHeader} onClick={() => setIsPromptExpanded(!isPromptExpanded)}>
                 <span style={{color: '#ffbf00'}}>✦</span> Senin Fikrin: {submittedPrompt.slice(0, 30)}...
               </div>
               {isPromptExpanded && <div style={userPromptBody}>{submittedPrompt}</div>}
             </div>

             {loading && !result ? (
               <div className="loading-box"><div className="loading-text">{loadingMessages[loadingStep]}</div></div>
             ) : (
                  ...
                  {!loading && result && (
                    <div style={{marginTop: '30px', borderTop: '1px solid #222', paddingTop: '20px'}}>
                      ...
                      <div style={{display: 'flex', flexWrap: 'wrap', gap: '10px', marginTop: '15px'}}>
                        {!isVisual ? (
                          <>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://chatgpt.com](https://chatgpt.com)", "ChatGPT")}>{IconChatGPT} ChatGPT</button>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://claude.ai](https://claude.ai)", "Claude")}>{IconClaude} Claude</button>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://gemini.google.com](https://gemini.google.com)", "Gemini")}>{IconGemini} Gemini</button>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://perplexity.ai](https://perplexity.ai)", "Perplexity")}>{IconPerplexity} Perplexity</button>
                          </>
                        ) : (
                          <>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://discord.com](https://discord.com)", "Midjourney")}>{IconMidjourney} Midjourney</button>
                            <button className="ai-brand-btn" onClick={() => hardRedirect("[https://leonardo.ai](https://leonardo.ai)", "Leonardo")}>{IconLeonardo} Leonardo</button>
                          </>
                        )}
                      </div>
                    </div>
      ...
      <div style={bottomArea}>
        <div style={inputBoxInner}>
          <textarea 
            style={inputField} placeholder={`Örn: "${typewriterText}"`} rows={1} 
            value={input} onChange={(e) => setInput(e.target.value)} 
            onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleGenerate(); }}}
          />
          <button onClick={handleGenerate} style={sendButton}>↑</button>
        </div>
      </div>
    </main>
  );
...
const topBar = { padding: '20px', position: 'absolute', top: 0, width: '100%', display: 'flex', justifyContent: 'space-between', zIndex: 100 };
const miniLogo = { height: '22px' };
const logoWrapper = { cursor: 'pointer' };
const backButton = { background: 'transparent', color: '#888', border: '1px solid #333', padding: '6px 12px', borderRadius: '20px', cursor: 'pointer' };
const contentArea = { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', paddingBottom: '120px' };
const floatingContainer = { position: 'absolute', top: 0, width: '100%', height: '70vh', pointerEvents: 'none' };
const heroSection = { textAlign: 'center', zIndex: 10, marginTop: '55vh' };
const centerLogo = { width: '140px', marginBottom: '20px' };
const heroTitle = { fontSize: '1.8rem', fontWeight: '600', color: '#fff' };
const heroSub = { color: '#666', fontSize: '0.9rem' };
const resultContainer = { maxWidth: '750px', width: '100%', zIndex: 10, padding: '0 20px' };
const userPromptWrapper = { backgroundColor: '#0f0f0f', borderRadius: '12px', border: '1px solid #222', marginBottom: '15px' };
const userPromptHeader = { padding: '12px 18px', cursor: 'pointer', fontSize: '0.85rem', color: '#888' };
const userPromptBody = { padding: '15px', borderTop: '1px solid #222', color: '#aaa', fontSize: '0.9rem' };
const aiResponseWrapper = { backgroundColor: '#0a0a0a', padding: '25px', borderRadius: '16px', border: '1px solid rgba(255, 191, 0, 0.2)' };
const aiLabel = { fontSize: '0.7rem', color: '#ffbf00', letterSpacing: '2px', marginBottom: '15px' };
const aiText = { fontSize: '1rem', lineHeight: '1.6', whiteSpace: 'pre-wrap', color: '#ddd' };
const copyBtn = { background: '#fff', color: '#000', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' };
const bottomArea = { position: 'fixed', bottom: '40px', width: '100%', display: 'flex', justifyContent: 'center', zIndex: 100 };
const inputBoxInner = { backgroundColor: '#0a0a0a', borderRadius: '30px', border: '1px solid rgba(255, 191, 0, 0.2)', display: 'flex', padding: '8px 15px', width: '90%', maxWidth: '600px', animation: 'elegantGlow 4s infinite' };
const inputField = { flex: 1, background: 'transparent', border: 'none', color: '#fff', outline: 'none', resize: 'none', padding: '10px' };
const sendButton = { width: '35px', height: '35px', borderRadius: '50%', background: '#fff', color: '#000', border: 'none', cursor: 'pointer', fontWeight: 'bold' };
